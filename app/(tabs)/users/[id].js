...
import { TouchableOpacity, View, Text, ActivityIndicator, Button } from 'react-native';
   ...
   const goToJupiter = () => {
    ...
    return (
        <View className='flex-1 items-center justify-center bg-yellow-100'>
            ...
            {isError && <Text className='text-red-500'>ERROR WHILE FETCHING DATA</Text>}
            <ActivityIndicator size={'large'} animating={isLoading}/>
            {/* <Button title='Refetch' onPress={refetch}/> */}
            <Button title='Login' onPress={() => login({email: 'testing@gmail.com', password: 'rahasia123'})}/>
        </View>
    );
}